function JointAngle = calLeftAnglesStand(trial,nFr)

%Calculate lower extrimity joint angle-----
HIP_ro = trial.PEL(:,1+4*(nFr-1):4+4*(nFr-1)) \ trial.lTHIGH(:,1+4*(nFr-1):4+4*(nFr-1));
KNEE_ro = trial.lTHIGH(:,1+4*(nFr-1):4+4*(nFr-1)) \ trial.lSHANK(:,1+4*(nFr-1):4+4*(nFr-1));
ANK_ro = trial.lSHANK(:,1+4*(nFr-1):4+4*(nFr-1)) \ trial.lFOOT(:,1+4*(nFr-1):4+4*(nFr-1));
SHOES_ro = trial.lFOOT(:,1+4*(nFr-1):4+4*(nFr-1));% \ ground;
PelFoot_ro = trial.PEL(:,1+4*(nFr-1):4+4*(nFr-1)) \ trial.lFOOT(:,1+4*(nFr-1):4+4*(nFr-1)); 
PEL_ro = trial.PEL(:,1+4*(nFr-1):4+4*(nFr-1));
ForeFOOT_ro = trial.lForeFOOT(:,1+4*(nFr-1):4+4*(nFr-1));
MPMP_ro = trial.lFOOT(:,1+4*(nFr-1):4+4*(nFr-1)) \ trial.lForeFOOT(:,1+4*(nFr-1):4+4*(nFr-1));
